...
public class QuestionScriptValidator {
    ...
    public ValidatedScript validate(
        ...
        for (int index = 0; index < sorted.size(); index++) {
            ...
            String normalizedText = signatureFactory.normalizeText(questionText);
            if (!normalizedTexts.add(normalizedText)) {
                throw ScriptGenerationException.invalidOutput(
                        "duplicate normalized question text");
            }

            String signature = signatureFactory.createSignature(
                    question.sourceType(),
        ...
        diversityPolicy.validateDiversity(validated, recentSessionIds, history);
    ...
    private String matchingJdExcerpt(String excerpt, String jobDescription) {
        if (excerpt == null || excerpt.isBlank()) {
            return null;
        }
        String normalized = requiredPlainText(excerpt, "sourceJdExcerpt", JD_EXCERPT_MAX_LENGTH);
        return signatureFactory.normalizeText(jobDescription)
                .contains(signatureFactory.normalizeText(normalized)) ? normalized : null;
    }
